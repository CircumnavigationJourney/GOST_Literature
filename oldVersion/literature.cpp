...
bool Literature::Load_data_from_file(QString &filepath){
...
void Literature::Save_data_to_file(QString &filepath){


}

void Literature::operate_str(QString& temp_str){
...
int Literature::authors_count(QString &authors){
  //Если авторов > 3
  //  QString eng = "[e";
  //  QString ru = "[и";
...
QString Literature::author_normalization(QString &author){
...
bool Literature::is_eng(QString &undefLangString) const {
...
// Функция приводит список в надлежащий вид,если строка уже содержит форматирование по ГОСТУ
//(на случай, если в ней есть ошибки) эта функция исправляет
//самые популярные ошибки при оформлении списка
void Literature::main_string_normalization(QString temp){
  ...
  QRegularExpression regex(tire);
  temp = temp.replace(regex,widetire);
      ...
      for (int i = 1; i < (*temp_string_list)[n].size(); ++i)
        {
          ...
          //СЛЕДУЮЩЕЕ УСЛОВИЕ не корректно тк срабатывает и в случаях, когда имя автора идет вначале, потом его инициалы, и сшивка
                   ...
                   && (*temp_string_list)[n][i+3].isUpper()) //это то из-за чего весь сыр бор
...
//============================================================================================================================================
QString Literature::generate_GOST(QString &authors, QString &name,QString &year,QString &T,QString &number, QString &pages,QString &jurnal){
...
void Literature::author_input_correction(QString &sourceString){
...
void Literature::title_input_correction(QString &title){
...
void Literature::year_input_correction(QString &year){
...
void Literature::number_input_correction(QString &number){
...
void Literature::volume_input_correction(QString &vol){
...
void Literature::pages_input_correction(QString &pages){
...
void Literature::jurnal_input_correction(QString &jurnal){
